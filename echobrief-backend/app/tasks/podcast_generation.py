    ...
    async with async_session() as session:
        ...
        for user in all_users:
            try:
                ...
                if user.plan_type == PlanType.FREE and len(topic_ids) > 3:
                ...
                todays_podcasts = await podcast_service._get_user_podcasts_today(user.id, today)
                ...
                podcast = await podcast_service.create_podcast_request(user.id, podcast_data, user)
                ...
                try:
                    ...
                    logger.error(f"Failed to generate script/audio for user {user.id}: {e}")
...
async def generate_podcast_for_user(user_id: str) -> dict:
    ...
    async with async_session() as session:
        ...
        # Create podcast
        podcast_data = PodcastCreate(topic_ids=topic_ids)
        podcast = await podcast_service.create_podcast_request(user_uuid, podcast_data, user)
