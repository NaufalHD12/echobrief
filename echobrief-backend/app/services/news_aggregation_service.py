...
class NewsAggregationService:
        ...
        for i, entry in enumerate(feed.entries):
            try:
                ...
                topic_id, generated_summary = await self._determine_topic_and_summary(
                    article_dict['title'],
                    article_dict['summary_text'] or ''
                )
                logger.debug(f"Entry {i}: AI result - topic_id: {topic_id}, has_summary: {bool(generated_summary)}")
                ...
                if topic_id and generated_summary:
                    ...
                    try:
                        ...
                        new_articles += 1
                        logger.info(f"Entry {i}: Created article - {article_dict['title'][:50]}...")
                    except HTTPException as e:
                        if "already exists" in str(e):
                            logger.debug(f"Entry {i}: Skipped duplicate article (detected during creation)")
                        else:
                            logger.error(f"Entry {i}: Error creating article: {e}")
                    except Exception as e:
                        logger.error(f"Entry {i}: Unexpected error creating article: {e}", exc_info=True)
                else:
    ...
    async def _determine_topic_and_summary(self, title: str, summary: str) -> tuple[Optional[int], str]:
            ...
            try:
                ...
                if content:
                    ...
                    if topic_id and summary_text:
                        logger.debug(f"AI success: topic_id={topic_id}, summary_length={len(summary_text)}")
                        return topic_id, summary_text
                    else:
                        logger.warning(f"AI response incomplete: topic_name={topic_name}, has_summary={bool(summary_text)}")
    ...
    def _fallback_topic_and_summary(self, title: str, summary: str, topics) -> tuple[Optional[int], str]:
        ...
        logger.info(f"Using fallback: topic_id={default_topic_id}, summary_length={len(simple_summary)}")
